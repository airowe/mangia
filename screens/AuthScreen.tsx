...
export const AuthScreen = ({ navigation }: any) => {
  ...
  const handleSignIn = useCallback(async () => {
    ...
    try {
      ...
      // Check if we need to complete first factor (password)
      if (signInAttempt.status === 'needs_first_factor') {
        ...
        if (result.status === 'complete') {
          ...
          // MFA is enabled - would need to handle 2FA here
          console.log('Sign in requires 2FA:', result);
          setError('Two-factor authentication required. Please contact support.');
        } else {
          console.log('Sign in result after first factor:', result);
          setError('Sign in incomplete. Please try again.');
        }
      } else if (signInAttempt.status === 'complete') {
        // Some configurations might complete immediately
        await setSignInActive({ session: signInAttempt.createdSessionId });
      } else {
        console.log('Sign in initial result:', signInAttempt);
