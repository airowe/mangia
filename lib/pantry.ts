  ...
  try {
    ...
    if (response.data && Array.isArray(response.data)) {
      return {
        ...
        totalPages: response.totalPages || Math.ceil((response.totalItems || 0) / (response.pageSize || limit)) || 1,
      };
    }
    ...
    if (response.data && response.data.data && Array.isArray(response.data.data)) {
      ...
      return {
        ...
        totalPages: data.totalPages || Math.ceil((data.totalItems || data.total || 0) / (data.pageSize || data.limit || limit)) || 1,
      };
    }
