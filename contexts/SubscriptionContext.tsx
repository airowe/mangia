...
import { DEV_USER_ID } from "../lib/devConfig";
...
/**
 * Dev-only subscription provider that doesn't use Clerk hooks.
 * Used when DEV_BYPASS_AUTH is enabled.
 */
export function DevSubscriptionProvider({ children }: SubscriptionProviderProps) {
  const [isPremium, setIsPremium] = useState(false);
  const [isLoading, setIsLoading] = useState(true);
  const [customerInfo, setCustomerInfo] = useState<CustomerInfo | null>(null);
  const [packages, setPackages] = useState<PurchasesPackage[]>([]);

  // Initialize RevenueCat without Clerk
  useEffect(() => {
    const initialize = async () => {
      try {
        await initializeRevenueCat(DEV_USER_ID);
...
/**
 * Production subscription provider that uses Clerk for auth.
 */
